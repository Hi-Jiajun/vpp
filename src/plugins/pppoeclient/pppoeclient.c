		...
		int is_broadcast)
{
   ...
   */
  {
    ...
    if ((packet_code == PPPOE_PADR || packet_code == PPPOE_PADS) && vec_len (c->cookie_value))
      {
	u16 cookie_len = vec_len (c->cookie_value);
	pppoe_tag_header_t *pppoe_tag = (pppoe_tag_header_t *) cursor;
			    ...
			    pppoeclient_history_cli_filter_t *filter)
{
  ...
  if (linux_ifname[0] != '\0' && linux_mac_present)
    vlib_cli_output (
      vm, "    access-link linux-ifname %s vpp-mac %U linux-mac %U mac-match %u", linux_ifname,
      format_ethernet_address,
      vnet_get_sup_hw_interface (pem->vnet_main, c->sw_if_index)->hw_address,
      format_ethernet_address, linux_mac,
      clib_memcmp (linux_mac,
		   vnet_get_sup_hw_interface (pem->vnet_main, c->sw_if_index)->hw_address, 6) == 0);
  else if (linux_ifname[0] != '\0')
    vlib_cli_output (vm, "    access-link linux-ifname %s vpp-mac %U linux-mac <unavailable>",
		     linux_ifname, format_ethernet_address,
		     vnet_get_sup_hw_interface (pem->vnet_main, c->sw_if_index)->hw_address);
  else
